import * as JSX9 from '@eslint-react/jsx';
import { createRuleForPlugin, normalizeSettings, decodeSettings, RE_JAVASCRIPT_PROTOCOL, findAttrInCustomAttributes } from '@eslint-react/shared';
import { O, F, isString, isNullable } from '@eslint-react/tools';
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        ...
        const elementType = JSX9.getElementType(jsxCtx, components, polymorphicPropName)(openingElementNameExpression);
        ...
        const hasAttr = (name2) => O.isSome(JSX9.findPropInAttributes(attributes, initialScope)(name2));
...
var no_dangerously_set_innerhtml_default = createRule({
  ...
  create(context) {
    return {
      JSXElement(node) {
        const initialScope = context.sourceCode.getScope(node);
        const maybeDanger = JSX9.findPropInAttributes(node.openingElement.attributes, initialScope)(
...
function firstChildIsText(node) {
  ...
  return node.children.length > 0 && !isNullable(firstChild) && !JSX9.isLineBreak(firstChild);
}
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const hasChildrenProp = () => JSX9.hasProp(node.openingElement.attributes, "children", initialScope);
        const hasDanger = () => JSX9.hasProp(node.openingElement.attributes, "dangerouslySetInnerHTML", initialScope);
...
var no_missing_button_type_default = createRule({
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        const jsxCtx = { getScope: (node2) => context.sourceCode.getScope(node2) };
        const elementType = JSX9.getElementType(jsxCtx, components, polymorphicPropName)(node.openingElement);
        if (elementType !== "button") return;
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const maybeTypeAttribute = JSX9.findPropInAttributes(attributes, initialScope)("type");
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        ...
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const maybeSandboxAttribute = JSX9.findPropInAttributes(attributes, initialScope)("sandbox");
        ...
        const hasValidSandbox = F.pipe(
          JSX9.getPropValue(sandboxAttribute, context.sourceCode.getScope(sandboxAttribute)),
  ...
  create(context) {
    return {
      JSXOpeningElement(node) {
        const name2 = JSX9.getElementName(node);
    ...
    return {
      JSXAttribute(node) {
        ...
        const isJavaScript = F.pipe(
          JSX9.getPropValue(node, context.sourceCode.getScope(node)),
          O.flatMapNullable((v) => v?.value),
          O.filter(isString),
    ...
    return {
      JSXElement(node) {
        ...
        const isSafeSandboxValue = !F.pipe(
          JSX9.getPropValue(maybeSandboxAttribute.value, context.sourceCode.getScope(maybeSandboxAttribute.value)),
          O.flatMapNullable(
            (v) => match(v?.value).with(P.string, F.identity).with({ sandbox: P.string }, ({ sandbox }) => sandbox).otherwise(F.constNull)
          ),
          O.filter(isString),
          O.map((value) => value.split(" ")),
          O.exists(
...
var no_unsafe_target_blank_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      const name2 = JSX9.getElementName(node.openingElement);
      const jsxCtx = { getScope: (node2) => context.sourceCode.getScope(node2) };
      const elementType = JSX9.getElementType(jsxCtx, components, polymorphicPropName)(node.openingElement);
      if (elementType !== "a" && !additionalComponents.some((c) => c.re.test(name2))) return O.none();
      const { attributes } = node.openingElement;
      const initialScope = context.sourceCode.getScope(node);
      ...
      const targetProp = JSX9.findPropInAttributes(attributes, initialScope)(targetPropName);
      const targetPropValue = O.isNone(targetProp) ? O.fromNullable(targetPropDefaultValue) : F.pipe(
        targetProp,
        O.flatMap((attr) => JSX9.getPropValue(attr, jsxCtx.getScope(attr))),
        O.flatMapNullable(
          (v) => match(v?.value).with(P.string, F.identity).with({ [targetPropName]: P.string }, (v2) => v2[targetPropName]).otherwise(F.constNull)
        ),
        O.filter(isString)
      );
      ...
      const hrefProp = JSX9.findPropInAttributes(attributes, initialScope)(hrefPropName);
      const hrefPropValue = O.isNone(hrefProp) ? O.fromNullable(hrefPropDefaultValue) : F.pipe(
        hrefProp,
        O.flatMap((attr) => JSX9.getPropValue(attr, jsxCtx.getScope(attr))),
        O.flatMapNullable(
          (v) => match(v?.value).with(P.string, F.identity).with(P.shape({ [hrefPropName]: P.string }), (v2) => v2[hrefPropName]).otherwise(F.constNull)
        ),
        O.filter(isString)
      );
      ...
      const relProp = JSX9.findPropInAttributes(attributes, initialScope)(relPropName);
      const relPropValue = O.isNone(relProp) ? O.fromNullable(relPropDefaultValue) : F.pipe(
        relProp,
        O.flatMap((attr) => JSX9.getPropValue(attr, jsxCtx.getScope(attr))),
        O.flatMapNullable(
          (v) => match(v?.value).with(P.string, F.identity).with(P.shape({ [relPropName]: P.string }), (v2) => v2[relPropName]).otherwise(F.constNull)
        ),
        O.filter(isString)
      );
