...
import { F, O, isString, MutRef, isNullable } from '@eslint-react/tools';
import { createRuleForPlugin, decodeSettings, unsafeReadSettings } from '@eslint-react/shared';
...
function check(node, context, allowExpressions) {
  ...
  if (allowExpressions && node.children.length === 1 && JSX7.isLiteral(firstChildren) && !isChildren) return;
